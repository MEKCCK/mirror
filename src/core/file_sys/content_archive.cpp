...
public:
    ...
    std::size_t GetSize() const override {
        return file.IsOpen() ? file.GetSize() : 0;
    }
...
static VirtualFile DecompressIfNCZ(VirtualFile file) {
    ...
    const auto ncz_file =
        file->IsNczFile() ? std::static_pointer_cast<NCZVirtualFile>(file) : nullptr;
    if (ncz_file && ncz_file->is_solid_stream) {
        ...
        const std::filesystem::path cache_path = temp_dir / (file->GetName() + ".v3.decompressed_cache");

        bool cache_valid = false;
        if (std::filesystem::exists(cache_path, ec)) {
            const std::size_t disk_size = std::filesystem::file_size(cache_path, ec);
            const std::filesystem::path completed_path = cache_path.string() + ".completed";
...
NCA::NCA(VirtualFile file_, const NCA* base_nca)
    ...
    if (rights_id != RightsId{}) {
        ...
        if (titlekey == Core::Crypto::Key128{}) {
            // Fallback: use the RightsId directly as the TitleKey when the
            // ticket/title.key is missing from the key database (covers
            // converted dumps re-encrypted with the rights id as key).
            std::memcpy(titlekey.data(), rights_id.data(), rights_id.size());
            LOG_WARNING(Loader,
                        "TitleKey not found! Falling back to using RightsId as TitleKey!");
        ...
        if (!is_fallback_key) {
            if (keys.HasKey(Core::Crypto::S128KeyType::Titlekek, master_key_id)) {
                ...
                Core::Crypto::AESCipher<Core::Crypto::Key128> cipher(titlekek,
